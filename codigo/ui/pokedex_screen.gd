## CódiceScreen — Registro de Tonales con lista y ficha detallada.
## Tonales no vistos aparecen como siluetas negras con "???".
## El registro se guarda en GameManager: seen_ids / caught_ids.
...
func _build_card(entry: Dictionary, seen: bool, caught: bool) -> PanelContainer:
	...
	if seen:
		var tex = _load_creature_sprite(str(entry["id"]))
		sprite.texture  = tex
	...
	if caught:
		...
		bond_lbl.text = "◈"
		bond_lbl.add_theme_color_override("font_color", Color(1.0, 0.85, 0.2))
		bond_lbl.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
		bond_lbl.add_theme_font_size_override("font_size", 8)
...
func _populate_detail(index: int) -> void:
	...
	for type_str in [entry.get("type1",""), entry.get("type2","")]:
		...
		var badge = Label.new()
		badge.text = PokedexData.esencia_es(type_str).to_upper()
	...
	# Estado de registro en el Códice
	var status_str = "SIN REGISTRO"
	if caught: status_str = "◈ VÍNCULO FORMADO"
	elif seen: status_str = "◦ AVISTADO"
	if detail_seen_txt:
		detail_seen_txt.text = status_str
		var sc = Color(1.0, 0.85, 0.2) if caught else (Color(0.5, 0.85, 0.5) if seen else Color(0.5, 0.5, 0.5))
		detail_seen_txt.add_theme_color_override("font_color", sc)

	# Base stats mini-bars
	for child in detail_stats.get_children(): child.queue_free()
	await get_tree().process_frame

	var base = CreatureStatSystem.get_base_stats(id)
	for kv in [["Aguante", "hp"], ["Ataque", "atk"], ["Defensa", "def"],
	           ["Atq.Esp", "spatk"], ["Def.Esp", "spdef"], ["Veloc.", "spd"]]:
		var label_txt = kv[0]
		var key       = kv[1]
...
func _update_count() -> void:
	...
	var total  := PokedexData.get_catalogue().size()
	count_label.text = "◦ Avistados: %d   ◈ Vínculos: %d / %d" % [seen, caught, total]

func _load_creature_sprite(creature_id) -> Texture2D:
	var id_int : int = creature_id if creature_id is int else int(str(creature_id))
	return RuntimeTextureLoader.load_nexo_sprite_by_id(id_int)
